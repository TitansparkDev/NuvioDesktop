...
object ServerDiscoveryService {
    ...
    suspend fun discover(input: String): Result<ServerConfiguration> = runCatching {
        ...
        val response = try {
            // callTimeoutMs rather than a bare withTimeout: the platform call blocks a thread, so
            // coroutine cancellation alone would not fire until it had already returned.
            httpRequestRaw(
                method = "GET",
                url = discoveryUrl,
                headers = mapOf(
                    "Accept" to "application/json",
                    "User-Agent" to "Desktop/${AppVersionConfig.VERSION_NAME.ifBlank { "dev" }}",
                ),
                body = "",
                callTimeoutMs = requestTimeoutMs,
            )
        ...
        if (response.isTruncated || response.body.encodeToByteArray().size > maxDocumentBytes) {
